    ...
    <div class="max-w-6xl mx-auto px-6 text-center">
        ...
        @if (isset($testimonials) && count($testimonials) > 0)
            ...
            <div id="main-review-card"
                ...
                <img id="review-image" src="{{ asset('assets/' . $defaultClient['image']) }}" alt="{{ $defaultClient['name'] }}"
                    class="w-24 h-24 rounded-full mx-auto border-4 border-blue-500 mb-4 object-cover shadow-lg" />
                <h3 id="review-name" class="font-semibold text-lg text-gray-800">{{ $defaultClient['name'] }}</h3>

                <div id="review-rating" class="flex justify-center mt-2 mb-4 text-yellow-400">
                    @for ($i = 0; $i < $defaultClient['rating']; $i++)
            ...
            <div class="flex flex-wrap justify-center gap-4">
                @foreach ($testimonials as $client)
                    ...
                    {{-- Tambahkan class 'testimonial-item' untuk listener dan data JSON --}}
                    <div class="testimonial-item flex items-center gap-3 rounded-full px-5 py-2 shadow-sm 
                        {{-- Atur styling default untuk klien pertama --}}
                        ...
                        data-client-data="{{ htmlspecialchars($clientData, ENT_QUOTES, 'UTF-8') }}">
                        ...
                        <div class="text-left">
                            ...
                            <span class="text-xs @if($client['id'] != $defaultClient['id']) text-gray-600 @else opacity-80 @endif testimonial-date">{{ $client['date'] }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
...
{{-- SKRIP JAVASCRIPT DINAMIS --}}
{{-- Menggunakan @push('scripts') agar skrip dijamin ditambahkan ke akhir body dari layouts/main.blade.php --}}
@push('scripts')
    <script>
