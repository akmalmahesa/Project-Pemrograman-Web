...
<div class="max-w-5xl mx-auto">
    ...
    <div class="bg-white shadow-lg rounded-2xl p-8 mb-8 border-l-4 border-green-500">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
            <div>
                ...
                <p class="text-2xl font-bold text-blue-600">#{{ $booking->id }}</p>
            </div>
            <div>
                <p class="text-gray-500 text-sm font-medium">Status</p>
                <p class="text-lg font-semibold text-green-600">
                    <i class="fas fa-check-circle mr-2"></i>{{ ucfirst($booking->status) }}
                </p>
            </div>
            <div>
                <p class="text-gray-500 text-sm font-medium">Durasi</p>
                <p class="text-lg font-semibold">{{ $booking->total_days }} hari</p>
            </div>
            <div>
            ...
            <div class="bg-white shadow-lg rounded-2xl overflow-hidden">
                ...
                <div class="bg-gray-300 h-80 flex items-center justify-center">
                    @if($booking->vehicle->image)
                        <img src="{{ asset('assets/' . $booking->vehicle->image) }}" 
                ...
                <div class="p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h3 class="text-3xl font-bold text-gray-900">{{ $booking->vehicle->name }}</h3>
                            <p class="text-gray-600 text-lg mt-1">
                                <i class="fas fa-tag mr-2"></i>{{ ucfirst($booking->vehicle->type) }}
                            </p>
                        </div>
                        @if($booking->vehicle->rating)
                            <div class="text-center">
                                <p class="text-3xl font-bold text-yellow-500">{{ $booking->vehicle->rating }}</p>
                    ...
                    <div class="grid grid-cols-2 gap-4 py-4 border-t border-b border-gray-200">
                        <div>
                            ...
                            <p class="font-semibold text-gray-900">
                                <i class="fas fa-cogs mr-2"></i>{{ ucfirst($booking->vehicle->transmission ?? 'Manual') }}
                            </p>
                        </div>
                        <div>
                            <p class="text-gray-600 text-sm">Harga/Hari</p>
                            <p class="font-semibold text-blue-600">
                                Rp{{ number_format($booking->vehicle->price_per_day, 0, ',', '.') }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 space-y-4">
                ...
                <div class="bg-white rounded-lg p-4 shadow-sm">
                    <div class="flex items-start">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="font-semibold text-gray-900 text-sm">{{ $booking->pickup_location }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg p-4 shadow-sm">
                    <div class="flex items-start">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="font-semibold text-gray-900 text-sm">{{ $booking->return_location }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="border-t pt-4 mt-4">
                    <div class="flex justify-between mb-2">
                        ...
                        <span class="font-semibold">Rp{{ number_format($booking->vehicle->price_per_day, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between mb-2">
                        <span class="text-gray-600">Jumlah Hari:</span>
                        <span class="font-semibold">{{ $booking->total_days }} hari</span>
                    </div>
                    <div class="flex justify-between mb-2 text-sm">
                        <span class="text-gray-600">Subtotal:</span>
                        <span class="font-semibold">Rp{{ number_format($booking->vehicle->price_per_day * $booking->total_days, 0, ',', '.') }}</span>
                    </div>
                    @if($booking->delivery_fee > 0)
                        <div class="flex justify-between mb-3 text-sm">
                            <span class="text-gray-600">Biaya Pengiriman:</span>
                            <span class="font-semibold text-green-600">Rp{{ number_format($booking->delivery_fee, 0, ',', '.') }}</span>
                        </div>
                    @else
                        <div class="flex justify-between mb-3 text-sm">
                            <span class="text-gray-600">Biaya Pengiriman:</span>
    ...
    <div class="flex flex-col md:flex-row gap-4 mt-12 justify-center">
        <a href="{{ route('rental.status', $booking->id) }}" 
