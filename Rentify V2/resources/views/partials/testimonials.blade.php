            ...
            <div id="mainCard" class="bg-white rounded-2xl border border-gray-100 shadow-md p-8">
                <div class="flex flex-col items-center">
                    <div class="w-28 h-28 rounded-full overflow-hidden border-4 border-blue-400 flex items-center justify-center">
                        <img id="mainAvatar" src="{{ asset('assets/client1.jpeg') }}" alt="client" class="w-full h-full object-cover">
                    </div>
            ...
            <div class="mt-4">
                <div class="flex gap-3 overflow-x-auto pb-2">
                    ...
                    @foreach($clients as $i => $c)
                        <button type="button" data-index="{{ $i }}" data-img="{{ $c['img'] }}" data-username="{{ $c['username'] }}" data-name="{{ $c['name'] }}" data-text="{{ $c['text'] }}" data-rating="{{ $c['rating'] }}" class="testimonial-thumb flex-shrink-0 w-48 sm:w-44 md:w-40 p-3 rounded-xl border border-gray-100 bg-white hover:shadow-md focus:outline-none" style="min-width: 12rem;">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-full overflow-hidden border-2 border-gray-200">
                                    <img src="{{ asset('assets/'.$c['img']) }}" alt="{{ $c['name'] }}" class="w-full h-full object-cover">
                                </div>
                                <div class="text-left">
                                    <div class="font-semibold text-sm">{{ $c['name'] }}</div>
                                    <div class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($c['date'])->format('d M Y') }}</div>
                                </div>
                            </div>
                        </button>
                    @endforeach
    ...
    <script>
        (function initTestimonials(){
            ...
            function setActive(index, {skipAutoplayReset = false} = {}) {
                ...
                const imgSrc = thumbImg ? thumbImg.src : (c.img ? (`{{ asset('assets/') }}` + c.img) : '');
